...
function borrar(){
    ...
    unset($nombre,$apellido_p,$apellido_m,$telefono);
  }
}
...
//boton html
$botonI=$html->button('x',['class' => 'btn btn-danger btn-block]']);
$Prueba=$html->input(['name'=>'eliminar','value'=>'eliminar','type'=>'submit','id'=>'id_eliminar','onclick'=>"borrar();"]);
...
if(empty($array_td))$html_table = '';
  ...
  $array_table['th'][] = ['No', 'Nombre', 'Ap.Paterno', 'Ap.Materno', 'Teléfono'];
